...
namespace banco.Model
{
    public class PessoaFisica : Cliente
    {
        ...
        public override bool EhMaior()
        {
            ...
            DateTime ObterIdadeMin = new DateTime(
                DataDeAniversario.Year + 18,
                DataDeAniversario.Month,
                DataDeAniversario.Day
            );
            if (hoje >= ObterIdadeMin)
            {
                Console.WriteLine(
                    "Cliente maior de idade, podemos prosseguir com abertura da conta."
                );
                return true;
            }
            else
            {
                Console.WriteLine(
                    "Cliente menor de idade, não podemos realizar a abertura de conta."
                );
                return false;
            }
        }
